"use strict";
var SessionReplayRecorder = (() => {
  var __defProp = Object.defineProperty;
  var __defNormalProp = (obj, key, value) => key in obj ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
  var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);

  // ../../node_modules/.pnpm/@rrweb+record@2.0.0-alpha.20/node_modules/@rrweb/record/dist/record.js
  ...
  var MutationBuffer = class {
    constructor() {
      ...
      __publicField2(this, "processMutation", (m) => {
  ...
  // ../../node_modules/.pnpm/@rrweb+rrweb-plugin-console-record@2.0.0-alpha.20_@rrweb+utils@2.0.0-alpha.20_rrweb@2.0.0-alpha.20/node_modules/@rrweb/rrweb-plugin-console-record/dist/rrweb-plugin-console-record.js
  ...
  function redactUrl(input2) {
    if (!input2)
      return input2;
    const isRelative = input2.startsWith("/");
    let url;
    try {
      url = new URL(input2, isRelative ? "https://session-replay.local" : void 0);
    } catch {
      return sanitizeVisibleText(input2);
    }
  ...
  function sanitizeVisibleText(value) {
    return value.replace(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi, "[email]").replace(/(?:\+?\d[\d\s().-]{7,}\d)/g, "[phone]").replace(/\b(?:\d[ -]*?){13,19}\b/g, "[card]");
  }
  ...
  function buildSanitizedDomSnapshot(documentRef, options) {
    ...
    for (const selector of options.ignoredSelectors) {
      clone.querySelectorAll(selector).forEach((node2) => node2.remove());
    }
    for (const selector of options.maskedSelectors) {
      clone.querySelectorAll(selector).forEach((node2) => maskNode(node2));
    }
    clone.querySelectorAll("input, textarea, select").forEach((node2) => maskNode(node2));
    clone.querySelectorAll("[value]").forEach((node2) => node2.removeAttribute("value"));
    clone.querySelectorAll("script, style, noscript").forEach((node2) => node2.remove());
    const html = truncate(collapseWhitespace(sanitizeVisibleText(clone.outerHTML)), 8e4);
    return {
  ...
  function createAiSnapshotPlugin(options) {
    return {
      name: AI_SNAPSHOT_PLUGIN,
      options: {},
      observer(callback, win) {
        ...
        emit();
        win.addEventListener("pagehide", emit);
        ...
        return () => {
          win.clearTimeout(timeout);
          win.removeEventListener("pagehide", emit);
  ...
  function maskNode(node2) {
    ...
    for (const attr of Array.from(node2.attributes)) {
      if (["value", "placeholder", "aria-label", "title"].includes(attr.name)) {
        node2.setAttribute(attr.name, "[masked]");
      }
    }
  }
  function removeComments(root2) {
    const walker = root2.ownerDocument?.createTreeWalker(root2, 128);
    if (!walker) return;
    const comments = [];
    while (walker.nextNode()) comments.push(walker.currentNode);
    comments.forEach((comment2) => comment2.parentNode?.removeChild(comment2));
  }
  ...
  // src/network.ts
  function installNetworkObserver(callback, win, ingestBaseUrl) {
    const restoreFetch = patchFetch(callback, win, ingestBaseUrl);
    const restoreXhr = patchXhr(callback, win, ingestBaseUrl);
    const restoreResourceTimings = observeResourceTimings(callback, win, ingestBaseUrl);
    return () => {
      restoreFetch();
      restoreXhr();
      restoreResourceTimings();
    };
  }
  function patchFetch(callback, win, ingestBaseUrl) {
    const fetchTarget = getFetchTarget(win);
    const original = fetchTarget?.fetch;
    if (!original) return () => void 0;
    const patchedFetch = async (input2, init) => {
      const request = normalizeFetchRequest(input2, init, win);
      if (shouldIgnoreUrl(request.url, ingestBaseUrl)) {
        return original.call(fetchTarget, input2, init);
      }
      const started = win.performance.now();
      try {
        const response = await original.call(fetchTarget, input2, init);
        callback({
          url: redactUrl(request.url),
          method: request.method,
          status: response.status,
          durationMs: win.performance.now() - started,
          ok: response.ok,
          size: Number(response.headers.get("content-length") ?? void 0) || void 0
        });
        return response;
      } catch (error) {
        callback({
  ...
  function patchXhr(callback, win, ingestBaseUrl) {
    const xhrConstructor = win.XMLHttpRequest;
    ...
    xhrConstructor.prototype.open = function open(method, url, ...args) {
      metadata.set(this, { method: method.toUpperCase(), url: new URL(String(url), win.location.href).toString(), startedAt: 0 });
      return originalOpen.call(this, method, url, ...args);
    };
    xhrConstructor.prototype.send = function send(body) {
      const item = metadata.get(this);
      if (!item || shouldIgnoreUrl(item.url, ingestBaseUrl)) {
        return originalSend.call(this, body);
      }
      item.startedAt = win.performance.now();
      const done = () => {
        callback({
          url: redactUrl(item.url),
          method: item.method,
          status: this.status,
          durationMs: win.performance.now() - item.startedAt,
          ok: this.status >= 200 && this.status < 400,
          size: Number(this.getResponseHeader("content-length") ?? void 0) || void 0
        });
    ...
    return () => {
      xhrConstructor.prototype.open = originalOpen;
      xhrConstructor.prototype.send = originalSend;
    };
  }
  function normalizeFetchRequest(input2, init, win) {
    const requestConstructor = win.Request ?? globalThis.Request;
    if (requestConstructor && input2 instanceof requestConstructor) {
      return {
        url: new URL(input2.url, win.location.href).toString(),
        method: (init?.method ?? input2.method ?? "GET").toUpperCase()
      };
    }
    return {
      url: new URL(String(input2), win.location.href).toString(),
      method: (init?.method ?? "GET").toUpperCase()
    };
  }
  function getFetchTarget(win) {
    const windowTarget = win;
    if (typeof windowTarget.fetch === "function") return windowTarget;
    if (typeof globalThis.fetch === "function") return globalThis;
    return null;
  }
  function shouldIgnoreUrl(url, ingestBaseUrl) {
    try {
      return new URL(url).origin === new URL(ingestBaseUrl).origin;
    } catch {
      return false;
    }
  }
  function observeResourceTimings(callback, win, ingestBaseUrl) {
    const seen = /* @__PURE__ */ new Set();
    const emit = (entry) => {
      if (!["fetch", "xmlhttprequest"].includes(entry.initiatorType)) return;
      if (shouldIgnoreUrl(entry.name, ingestBaseUrl)) return;
      ...
      callback({
        ...
        ok: true,
        size: entry.transferSize || entry.encodedBodySize || void 0
      });
    };
    win.performance.getEntriesByType("resource").forEach((entry) => emit(entry));
    const Observer = win.PerformanceObserver ?? globalThis.PerformanceObserver;
    if (!Observer) return () => void 0;
    const observer = new Observer((list2) => {
      list2.getEntries().forEach((entry) => emit(entry));
    });
    try {
  ...
  var EventTransport = class {
    ...
    async init() {
      ...
      if (stored) {
        ...
        this.sequence = stored.nextSequence;
        this.timer = window.setInterval(() => void this.flush(), 1e4);
        return;
      }
      const response = await fetch(`${this.config.ingestBaseUrl}/v1/sessions/init`, {
        method: "POST",
      ...
      this.sequence = this.session.nextSequence;
    ...
    async flush(useBeacon = false) {
      ...
      const task = this.flushNow(useBeacon);
      this.flushing = task;
      try {
        await task;
      } finally {
    ...
    async flushNow(useBeacon = false) {
      ...
      const url = `${this.config.ingestBaseUrl}/v1/sessions/${encodeURIComponent(this.session.sessionId)}/events`;
    ...
    async flushPagehide() {
      this.addLifecycleEvent("pagehide");
      await this.flush(true);
    }
    async end() {
      this.addLifecycleEvent("end");
      this.stopped = true;
      ...
      const url = `${this.config.ingestBaseUrl}/v1/sessions/${encodeURIComponent(this.session.sessionId)}/end`;
    ...
    readStoredSession() {
      try {
        ...
        if (stored.siteId !== this.config.siteId || !stored.sessionId || !stored.token || typeof stored.nextSequence !== "number") return null;
        if (typeof stored.createdAt !== "number" || Date.now() - stored.createdAt > 30 * 6e4) return null;
        return stored;
      } catch {
        return null;
      }
  ...
  async function startRecorder() {
    ...
    if (!config || window.__SESSION_REPLAY_CONTEXT__?.consentDenied) return;
    const context = window.__SESSION_REPLAY_CONTEXT__ ?? { userId: null, tags: {}, consentDenied: false };
    const transport = new EventTransport(config, context);
    await transport.init();
    const stopRecord = record({
      emit: (event) => transport.add(event),
      maskAllInputs: true,
      maskInputOptions: {
        color: false,
        date: true,
      ...
      maskTextSelector: config.maskedSelectors.join(","),
      ignoreSelector: config.ignoredSelectors.join(","),
      slimDOMOptions: "all",
      inlineStylesheet: true,
      collectFonts: true,
      sampling: {
    ...
    window.addEventListener("pagehide", () => void transport.flushPagehide());
    window.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "hidden") void transport.flushPagehide();
    });
    drainQueuedCommands(window.SessionReplay?._queue ?? [], window.SessionReplay);
  }
  function installLiveApi(config, transport, stop) {
    const context = window.__SESSION_REPLAY_CONTEXT__ ?? { userId: null, tags: {}, consentDenied: false };
    const api = {
      grantConsent: () => {
        context.consentDenied = false;
      },
      denyConsent: () => {
        context.consentDenied = true;
        stop();
      },
      stop,
      setUser: (userId) => {
      ...
      captureNetwork: (payload) => {
        record.addCustomEvent("session-replay/network@1", payload);
      }
    };
    window.__SESSION_REPLAY_CONTEXT__ = context;
    window.__SESSION_REPLAY_CONFIG__ = config;
    window.SessionReplay = api;
    void transport.flush();
  }
  function drainQueuedCommands(commands, api) {
    if (!api) return;
    for (const command of commands) {
      if (command.type === "grantConsent") api.grantConsent();
      if (command.type === "denyConsent") api.denyConsent();
      if (command.type === "stop") api.stop();
      if (command.type === "setUser") api.setUser(command.userId);
      if (command.type === "setTags") api.setTags(command.tags);
      if (command.type === "captureNetwork") api.captureNetwork?.(command.payload);
    }
  }
})();
